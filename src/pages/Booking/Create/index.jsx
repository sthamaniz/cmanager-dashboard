...
import useBookingCreate from 'hooks/booking/useBookingCreate';
...
export default ({ history }) => {
  ...
  const submitFormData = (formData) => {
    if (!bookingDays.length) {
      ...
      setError('');
      setLoading(true);
      bookingCreateTrigger(bookingData);
    }
  };
  ...
  useEffect(() => {
    if (!bookingCreateLoading) {
      if (bookingCreateError) {
        const errorMessage =
          bookingCreateError.message || 'Something went wrong.';
  ...
  }, [bookingCreateLoading, bookingCreateResult]);
